function parseData(data) {
  ...
  for (const element of dataList) {
    const [town, rainfallData] = element.split(':');
    const rainfallDataList = rainfallData.split(',');
    const rainMetricList = [];
    for (const rainfallData of rainfallDataList) {
      const [_month, rainMetric] = rainfallData.split(' ');
      rainMetricList.push(+rainMetric);
    }

    hashMap.set(town, rainMetricList);
  }
...
function mean(town, data) {
  const hashMap = parseData(data);
  const rainMetricList = hashMap.get(town);
  if (!rainMetricList) return -1;
  let sum = rainMetricList.reduce((acc, current) => {
    return acc + current;
  }, 0);
...
function variance(town, data) {
  ...
  if (!rainMetricList) return -1;
  let variance = 0;
  for (const rainMetric of rainMetricList) {
    variance += Math.pow(rainMetric - meanValue, 2);
  }
